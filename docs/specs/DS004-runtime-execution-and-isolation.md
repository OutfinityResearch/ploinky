...
The runtime may also execute agents through sandbox backends such as `bwrap` and `seatbelt`. The actual backend used for a running service must be recorded in the workspace registry, because a manifest’s preferred runtime may not be the runtime that ultimately succeeded after fallback handling.
...
Ploinky’s runtime layer must continue to provide predictable service startup across container and sandbox backends, preserve the shared `Agent/` payload, and apply profile-aware isolation rules that are visible to operators and tests.
