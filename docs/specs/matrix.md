...
| [DS006](/specsLoader.html?spec=DS006-auth-capabilities-and-secure-wire.md) | Auth, Capabilities, and Secure Wire | [[status:implemented-(wire-protocol-superseded-by-ds011)]] | ploinky-team | Defines local auth, SSO provider binding, capability discovery, and the signed secure-wire path for delegated agent calls. |
