...
The WebChat composer autocomplete file-suggestion endpoint (`/webchat/suggestions/files`) must apply the same workspace-confinement rules as Explorer's filesystem reads. The endpoint rejects absolute caller paths, traversal (`..`), NUL bytes, symlink escapes, and the reserved secret files `.secrets` and `*.secrets`. It must not return host absolute paths to the browser. The endpoint provides suggestions only; downstream chat agents must still validate any structured `workspace-path` references they receive before reading file content. WebChat must sanitize references on the way out (drop absolute paths, traversal, NUL bytes, reserved secret-file names, and any non-string path values) so an injected envelope cannot smuggle arbitrary host paths to a tag-relay agent.

Browser autocomplete must not treat URL query parameters as authority to call arbitrary MCP tools. Tag suggestions may use static launch data such as `tag-relay-tags`; dynamic backend discovery belongs to the selected chat agent or relay during a real delegated request that carries a router-minted invocation token.
