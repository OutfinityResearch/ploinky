...
`PLOINKY_MASTER_KEY` is the root cryptographic secret. It is consumed as 256 bits of key material. `cli/services/masterKey.js` accepts the value in two forms: a 64-character hexadecimal string is decoded directly as raw key bytes, and any other non-empty string is treated as a seed and hashed to 32 bytes with SHA-256. The hex form is preferred when high-entropy random material is available; the seed form exists so operators can use a memorable passphrase, but its strength is bounded by the entropy of the chosen string. Resolution checks `process.env.PLOINKY_MASTER_KEY` first and then falls back to the nearest `.env` walked upward from the current working directory, so operators can keep one `.env` in a parent directory that shadows multiple workspaces. Trust implication: anyone who can read the resolved `.env` file can decrypt every encrypted store under any descendant workspace; the file's filesystem permissions therefore inherit the same trust level as `PLOINKY_MASTER_KEY` itself.
