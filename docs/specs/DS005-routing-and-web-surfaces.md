...
For `/webchat`, the router must treat `agent` as an explicit agent-selection query parameter and must preserve the remaining query parameters across the browser session endpoints. When a WebChat request selects an explicit agent, the router must forward every additional query parameter except router-reserved stream/session parameters such as `tabId` to that agent's `ploinky cli <agent>` launch as long-form CLI flags encoded as single `--key=value` tokens. The router must not hardcode parameter names for this forwarding behavior; interpretation belongs to the target agent CLI.

WebChat must provide a slash-command autocomplete surface. When the user types `/` in the composer, the client must query the agent's MCP endpoint (`/mcps/<agent>/mcp`) for available tools, map `execute_<skill>` tool names to slash commands, and present a filterable dropdown menu. If an agent exposes a structured command-catalog MCP tool, WebChat may send preserved launch query parameters such as `dir` to that tool and must consume any declared `subCommands` and generic `argCompletions` without hardcoding agent-specific command names. Arrow keys navigate the menu; Enter or Tab inserts the selected command or argument completion into the input without sending it. If the MCP catalog fetch fails or returns no tools, the autocomplete surface remains silent (no error, no menu).
